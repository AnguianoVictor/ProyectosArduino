...
//IPAddress ip(10 , 2, 0, 1); //--Suc PO Cuautitlan--
...
//IPAddress ip(192, 168, 8, 30); //Suc Nicolas Romero 2  
IPAddress ip(192, 168, 219, 30); //Suc Cuautitlan 2             
          ...
          while (cliente.connected()) {                           //Mientra el cliente este conectado
            if (cliente.available()) {                              //Si el cliente esta disponible
              ...
              int posicion=cadena.indexOf("LED=");                    //Guardamos la posición de la instancia "LED=" a la variable 'posicion'
                ...
                if(cadena.substring(posicion)=="LED=ONMU")
                  {
                    MagnetUnoON();  
                  }
                if(cadena.substring(posicion)=="LED=OFFMU")
                  {
                    MagnetUnoOFF();  
                  }
                if(cadena.substring(posicion)=="LED=ONMD")
                  {
                    MagnetDosON();  
                  }
                if(cadena.substring(posicion)=="LED=OFFMD")
                  {
                    MagnetDosOFF();
                  }
